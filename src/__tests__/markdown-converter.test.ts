...
describe('MarkdownConverter', () => {
  ...
  describe('convertToTelegraphNodes', () => {
    it('should convert markdown to Telegraph nodes', () => {
      ...
      const mockHtml = '<h1>Hello World</h1><p>This is <strong>bold</strong> text.</p>';
    ...
    it('should handle conversion errors', () => {
      ...
      expect(() => converter.convertToTelegraphNodes(markdown)).toThrow('Conversion error');
    });
  });
  ...
  describe('convertToTelegraphNodesSimple', () => {
    it('should convert simple markdown headers', () => {
      ...
      expect(result[0]).toEqual({ tag: 'h3', children: ['H1 Title'] });
      expect(result[1]).toEqual({ tag: 'h3', children: ['H2 Title'] });
      expect(result[2]).toEqual({ tag: 'h4', children: ['H3 Title'] });
    });
    ...
    it('should convert blockquotes', () => {
      ...
      expect(result).toHaveLength(2);
      expect(result[0]).toEqual({ tag: 'blockquote', children: ['This is a quote'] });
      expect(result[1]).toEqual({ tag: 'blockquote', children: ['Second line'] });
    });
    ...
    it('should convert regular paragraphs with inline formatting', () => {
      ...
      expect(result[0].children?.[0]).toContain('<strong>bold</strong>');
      expect(result[0].children?.[0]).toContain('<em>italic</em>');
      expect(result[0].children?.[0]).toContain('<code>code</code>');
    });
    ...
    it('should convert links', () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0].tag).toBe('p');
      expect(result[0].children?.[0]).toContain('<a href="https://google.com">Google</a>');
    });
    ...
    it('should skip empty lines', () => {
      ...
      expect(result).toHaveLength(2);
      expect(result[0]).toEqual({ tag: 'h3', children: ['Title'] });
      expect(result[1]).toEqual({ tag: 'p', children: ['Content here'] });
    });
    ...
    it('should handle frontmatter removal', () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0]).toEqual({ tag: 'h3', children: ['Real Content'] });
    });
  ...
  describe('processInlineMarkdown', () => {
    ...
    it('should process links', () => {
      ...
      expect(result).toBe('Visit <a href="https://google.com">Google</a> website');
    });

    it('should process multiple formatting types', () => {
      const text = '**Bold** and *italic* with `code` and [link](https://example.com)';
      
      const result = (converter as any).processInlineMarkdown(text);
      
      expect(result).toBe('<strong>Bold</strong> and <em>italic</em> with <code>code</code> and <a href="https://example.com">link</a>');
    });
    ...
    it('should process internal markdown links with resolver', () => {
      ...
      const result = (converter as any).processInlineMarkdown(text, basePath, linkResolver);
      
      expect(result).toBe('Check <a href="https://telegra.ph/Guide-123">guide</a> for details.');
    });
